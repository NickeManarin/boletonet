...
namespace BoletoNet.Arquivo.Reader
{
    ...
    {
        private readonly NumberFormatInfo _numberFormat = CultureInfo.GetCultureInfo("pt-BR").NumberFormat;

        public LinhaCbr643 Read(string line)
        {
            var type = typeof(LinhaCbr643);

            var lineObject = GetType().Assembly.GetTypes().Where(t => !t.IsAbstract && type.IsAssignableFrom(t))
                .Select(t => (LinhaCbr643)Activator.CreateInstance(t)).FirstOrDefault(t => t.LinhaCorresponde(line));

            var tuples = lineObject.GetType().GetProperties()
                .Where(p => p.CanWrite).Select(p => new Tuple<PropertyInfo, TextPosAttribute>(p, p.GetFirstAttribute<TextPosAttribute>()))
                .Where(t => t.Item2 != null).ToList();
            ...
            foreach (var tuple in tuples)
            {
                ...
                if (typeof(DateTime).IsAssignableFrom(tuple.Item1.PropertyType))
                {
                    ...
                    tuple.Item1.SetValue(lineObject, int.Parse(str, _numberFormat), null);
                else if (typeof(decimal).IsAssignableFrom(tuple.Item1.PropertyType))
                {
                    ...
                    tuple.Item1.SetValue(lineObject, decimal.Parse(str, _numberFormat), null);
                }
                else
