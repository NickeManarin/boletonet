...
namespace BoletoNet
{
    ...
    internal class Banco_Brasil : AbstractBanco, IBanco
    {
        ...
        public override void FormataLinhaDigitavel(Boleto boleto)
        {
            ...
            var cmplivre = Strings.Mid(boleto.CodigoBarra.Codigo, 20, 25);

            var campo1 = Strings.Left(boleto.CodigoBarra.Codigo, 4) + Strings.Mid(cmplivre, 1, 5);
             ...
             */
            var campo2 = Strings.Mid(cmplivre, 6, 10);
             ...
             */
            var campo3 = Strings.Mid(cmplivre, 16, 10);
             ...
             */
            var campo4 = Strings.Mid(boleto.CodigoBarra.Codigo, 5, 1);
            ...
            var campo5 = icampo5 == 0 ? "000" : icampo5.ToString();
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio)
        {
            try
            {
                ...
                segmentoP += " "; // jefhtavares O banco não aceita mais esse campo como 0 (zero), o campo deverá ser enviado em branco
                ...
                switch (boleto.Cedente.Convenio.ToString().Length)
                {
                    case 4:
                        ...
                        nossoNumero = string.Format("{0}{1}{2}", boleto.Cedente.Convenio, Utils.FormatCode(boleto.NossoNumero, 7), Mod11BancoBrasil(boleto.NossoNumero));
                        break;
                    case 6:
                        ...
                        if (boleto.NossoNumero.Length != 11)
                            nossoNumero = string.Format("{0}{1}{2}", boleto.Cedente.Convenio, Utils.FormatCode(boleto.NossoNumero, 5), Mod11BancoBrasil(boleto.NossoNumero));
                        else
                            nossoNumero = boleto.NossoNumero + Mod11BancoBrasil(boleto.NossoNumero);
                        break;
                    case 7:
                        ...
                        if (boleto.NossoNumero.Length != 17)
                            nossoNumero = string.Format("{0}{1}", boleto.Cedente.Convenio, Utils.FormatCode(boleto.NossoNumero, 10)); //sao 10 digitos pro no. sequencial
                        else
                ...
                segmentoP += Utils.FitStringLength(nossoNumero, 20, 20, ' ', 0, true, true, false);
                ...
                {
                    segmentoP += boleto.ModalidadeCobranca.ToString();
                }
                ...
                segmentoP += Utils.FitStringLength(boleto.ValorBoleto.ToString("f").Replace(",", "").Replace(".", ""), 15, 15, '0', 0, true, true, true);
                segmentoP += "00000 ";
                ...
                if (boleto.ValorDesconto > 0)
                {
                    ...
                    segmentoP += "000000000000000000000000";

                segmentoP += "000000000000000";
                segmentoP += "000000000000000";
                segmentoP += Utils.FitStringLength(boleto.NumeroDocumento, 25, 25, '0', 0, true, true, true);

                //alterado por marcelhsouza em 28/03/2013
                //O Banco do Brasil trata somente os códigos '1' – Protestar dias corridos, '2' – Protestar dias úteis, e '3' – Não protestar.
                var codigoProtesto = "3";
                var diasProtesto = "00";
                ...
                segmentoP += codigoProtesto;
                segmentoP += diasProtesto;

                /*if (boleto.Instrucoes.Count > 1 && boleto.Instrucoes[0].Dias > 0)
                {
                    _segmentoP += "2";
                    _segmentoP += Utils.FitStringLength(boleto.Instrucoes[0].Dias.ToString(), 2, 2, '0', 0, true, true, true);
                }
                else
                    _segmentoP += "300";*/
        ...
        public override string GerarTrailerArquivoRemessa(int numeroRegistro)
        {
            try
            {
                var brancos205 = new string(' ', 205);

                var trailerArquivo = "00199999         000001";
                trailerArquivo += Utils.FitStringLength((numeroRegistro + 1).ToString(), 6, 6, '0', 0, true, true, true); //deve considerar 1 registro a mais - Header
                trailerArquivo += "000000";
                trailerArquivo += brancos205;

                trailerArquivo = Utils.SubstituiCaracteresEspeciais(trailerArquivo);
        ...
        public override DetalheSegmentoTRetornoCNAB240 LerDetalheSegmentoTRetornoCNAB240(string registro)
        {
            ...
            var _outUsoExclusivo = registro.Substring(37, 9); //13
            var _outNossoNumero = registro.Substring(37, 20); //14
            var _outCarteira = registro.Substring(57, 1); //15
            var _outNumeroDocumento = registro.Substring(58, 15); //16
            var _outVencimento = registro.Substring(73, 8); //17
            var _outValor = registro.Substring(81, 15); //18
            var _outUf = registro.Substring(96, 2); //19
            var _outBanco = registro.Substring(98, 1); //20
            var _outCodCedente = registro.Substring(99, 5); //21
            var _outDvCedente = registro.Substring(104, 1); //22
            var _outNomeCedente = registro.Substring(105, 25); //23
            var _outCodMoeda = registro.Substring(130, 2); //24
            var _sacadoInscricaoTipo = registro.Substring(132, 1); //25
            var _sacadoInscricaoNumero = registro.Substring(133, 15); //26
            var _sacadoNome = registro.Substring(148, 40); //27
            var _cnab28 = registro.Substring(188, 10); //28
            var _valorTarifasCustas = registro.Substring(198, 13); //29
            var _motivoCobraca = registro.Substring(213, 10); //30
            var _cnab31 = registro.Substring(223, 17); //31
            ...
            try
            {
                ...
                segmentoT.DataVencimento = registro.Substring(73, 8) == "00000000" ? DateTime.Now : DateTime.ParseExact(registro.Substring(73, 8), "ddMMyyyy", CultureInfo.InvariantCulture); //17
        ...
        public override DetalheSegmentoURetornoCNAB240 LerDetalheSegmentoURetornoCNAB240(string registro)
        {
            ...
            var _dadosTituloValorPago = registro.Substring(76, 15); //12
            var _dadosTituloValorCreditoBruto = registro.Substring(92, 15); //13
            var _outDespesas = registro.Substring(107, 15); //14
            var _outPerCredEntRecb = registro.Substring(122, 5); //15
            var _outBanco = registro.Substring(127, 10); //16
            var _outDataOcorrencia = registro.Substring(137, 8); //17
        ...
        public string GerarHeaderRemessaCnab400(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediAlphaAliEsquerda_____, 0003, 007, 0, "REMESSA", ' '));                             //003-009 "TESTE"
                ...
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediAlphaAliEsquerda_____, 0077, 018, 0, "001BANCODOBRASIL", ' '));                    //077-094
                ...
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediNumericoSemSeparador_, 0130, 007, 0, cedente.Convenio, '0'));                      //130-136
        ...
        public string GerarDetalheRemessaCnab400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediAlphaAliEsquerda_____, 0032, 007, 0, boleto.Cedente.Convenio, ' '));                   //032-038
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediAlphaAliEsquerda_____, 0039, 025, 0, boleto.NumeroDocumento, ' '));                    //039-063
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediNumericoSemSeparador_, 0064, 017, 0, boleto.NossoNumero, '0'));                        //064-080
                ...
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediNumericoSemSeparador_, 0107, 002, 0, boleto.Cedente.Carteira, '0'));                   //107-108
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediAlphaAliEsquerda_____, 0109, 002, 0, ObterCodigoDaOcorrencia(boleto), ' '));           //109-110
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediAlphaAliEsquerda_____, 0111, 010, 0, boleto.NumeroDocumento, '0'));                    //111-120
                ...
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediAlphaAliEsquerda_____, 0140, 003, 0, "001", '0'));                                     //140-142   
                ...
                reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediNumericoSemSeparador_, 0148, 002, 0, especie, '0'));                                   //148-149
                ...
                if (boleto.Remessa.CodigoOcorrencia.Equals("35") || boleto.Remessa.CodigoOcorrencia.Equals("36"))   //“35” – Cobrar Multa – ou “36” - Dispensar Multa 
                {
                    ...
                    reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediNumericoSemSeparador_, 0174, 001, 0, vCodigoMulta, '0'));                          //174 a 174      Código da Multa 1=Valor 
                    reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediDataDDMMAA___________, 0175, 006, 0, vDataVencimento, ' '));                       //175 a 180      Data de inicio para Cobrança da Multa 
                    reg.CamposEdi.Add(new CampoRegistroEdi(TiposDadoEdi.ediNumericoSemSeparador_, 0180, 013, 2, vMulta, '0'));                                //181 a 192      Valor de Multa 
                }
                else
                {
                    #region DataDesconto
